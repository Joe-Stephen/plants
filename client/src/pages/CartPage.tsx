import { useGetCartQuery, useUpdateCartItemMutation, useRemoveFromCartMutation } from '../features/cart/cartApi';
  ...
  if (error) {
    return (
      <div className="text-center py-16">
        ...
        <Link to="/login" className="bg-primary-600 text-white px-6 py-2 rounded-lg hover:bg-primary-700">
          Login
        </Link>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden mb-8">
        {cartItems.map((item) => (
          <div key={item.id} className="p-6 border-b border-gray-100 flex items-center gap-6">
            <img 
              src={item.product.images.find(img => img.isPrimary)?.url || 'https://via.placeholder.com/100'} 
              alt={item.product.name} 
              className="w-24 h-24 object-cover rounded-lg bg-gray-100"
            />
            
            <div className="flex-grow">
              <Link to={`/shop/${item.product.id}`} className="font-bold text-lg hover:text-primary-600">
                {item.product.name}
              </Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={() => updateItem({ id: item.id, quantity: Math.min(item.product.stock, item.quantity + 1) })}
                className="p-1 text-gray-400 hover:text-gray-600 border rounded"
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 bg-white p-6 rounded-xl shadow-sm">
        <div className="text-gray-600">
           <Link to="/shop" className="hover:underline">&larr; Continue Shopping</Link>
        </div>
