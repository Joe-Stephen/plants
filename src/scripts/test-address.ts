...
const createAddress1 = async () => {
    try {
        ...
        address1Id = (res.data as any).data.address.id;
...
const createAddress2 = async () => {
    try {
        ...
        address2Id = (res.data as any).data.address.id;
...
const verifyDefault = async () => {
    try {
        ...
        const addresses = (res.data as any).data.addresses;
        const addr1 = addresses.find((a: any) => a.id === address1Id);
        const addr2 = addresses.find((a: any) => a.id === address2Id);
...
const updateDefault = async () => {
    try {
        ...
        await axios.put(`${API_URL}/addresses/${address2Id}`, {
            is_default: true
        }, {
            headers: { Authorization: `Bearer ${token}` }
        });
...
const deleteAddress = async () => {
    try {
        ...
        await axios.delete(`${API_URL}/addresses/${address1Id}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
